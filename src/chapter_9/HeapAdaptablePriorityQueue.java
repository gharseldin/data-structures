...
public class HeapAdaptablePriorityQueue<K,V> extends  HeapPriorityQueue<K,V> {
	...
	/**
	 * Restores the head property by moving the entry at index j upward/downward
	 */
	protected void bubble(int j){
		if(j>0 && compare(heap.get(j), heap.get(parent(j))) < 0)
			upheap(j);
		else
			downheap(j);	// although it might not need to
	}
	...
	public void remove (Entry<K,V> entry) throws IllegalArgumentException{
		...
		int j = locator.getIndex();
		if(j == heap.size()-1)				// entry is at the last position
